...
class DprnnBlock(keras.layers.Layer):
    ...
    def call(self, T):
        ...
        T_shaped = tf.reshape(T, (self.batch_size*self.num_overlapping_chunks, self.chunk_size,
        ...
        U_Hat = tf.reshape(U_Hat, (self.batch_size,
                                   self.num_overlapping_chunks*self.chunk_size*self.num_filters_in_encoder))
        LN_U_Hat = self.intra_ln(U_Hat)
        LN_U_Hat = tf.reshape(LN_U_Hat, (self.batch_size, self.num_overlapping_chunks, self.chunk_size,
                                         self.num_filters_in_encoder))
        T_Hat = T + LN_U_Hat

        # Inter-Chunk Processing
        T_Hat = tf.transpose(T_Hat, [0, 2, 1, 3])
        T_Hat_shaped = tf.reshape(T_Hat, (self.batch_size*self.chunk_size, self.num_overlapping_chunks,
                                          self.num_filters_in_encoder))
